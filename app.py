...
if 'current_analysis' not in res_st.session_state:
    res_st.session_state.current_analysis = None # Ana sayfada geçici analiz sonucunu tutmak için
...
with res_st.sidebar:
    if not res_st.session_state.logged_in:
        ...
        u_info = res_st.mock_users_db[res_st.session_state.user_email]
    ...
    if sayfa == "🏠 Ana Sayfa (CV Analiz)":
        ...
        with col_left:
                ...
                if res_st.button("🚀 Analiz Et", type="primary"):
                    if u_info["kalan_hak"] > 0:
                        with res_st.spinner("Analiz ediliyor..."):
                            # Sadece geçici hafızaya alıyoruz, direkt listeye eklemiyoruz
                            res_st.session_state.current_analysis = analyze_cv_mock(uploaded_file.name)
                            res_st.rerun()
                    else:
                        res_st.error("❌ Limitiniz bitti!")

            # 🟢 EĞER ANALİZ YAPILDIYSA BURASI AÇILACAK (KAYDETME VE DEĞERLENDİRME ALANI)
            if res_st.session_state.current_analysis is not None:
                ...
                # 2. Ana Sayfadan Olumlu/Olumsuz/Nötr İşaretleme Alanı
                ...
                # Gerçek değer haritalaması
                durum_clean = ana_secilen_durum.split(" ")[1] 
                
                # 3. KESİN KAYIT BUTONU
                if res_st.button("💾 Havuza Güvenle Kaydet", type="secondary"):
                    ...
                    # Ana listeye şimdi ekliyoruz
                    res_st.session_state.mock_cvs_db.append(final_cv)
                    
                    # Kotayı düşüyoruz
                    if u_info['paket_turu'] != "Sınırsız (Kurumsal)":
                        res_st.mock_users_db[res_st.session_state.user_email]["kalan_hak"] -= 1
                    
                    res_st.success(f"💾 CV, {current_emoji if (current_emoji := {'Yeni': '🔵', 'Olumlu': '🟢', 'Nötr': '🟡', 'Olumsuz': '🔴'}.get(durum_clean)) else '🔵'} etiketiyle '{ana_secilen_kat}' kategorisine başarıyla kaydedildi!")
                    res_st.session_state.current_analysis = None # Ekranı temizle
                    time.sleep(1)
                    res_st.rerun()
        ...
        with col_right:
            ...
            if res_st.session_state.current_analysis is not None:
                ...
                res_st.info("👆 Sol taraftaki panelden kategoriyi ve durum rengini seçip 'Havuza Güvenle Kaydet' butonuna basarak işlemi tamamlayabilirsiniz.")
            else:
